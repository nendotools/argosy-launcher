...
void VideoLayout::updateForegroundVertices() {
    ...
    LOGD(
        "Computing foreground vertices from screen (%d x %d), content (%d x %d), aspect ratio (%f), integer scaling (%s), rotation (%f radians)",
        screenWidth,
        screenHeight,
        contentWidth,
        contentHeight,
        aspectRatio,
        integerScaling ? "true" : "false",
        rotation
    );
    ...
    if (integerScaling && contentWidth > 0 && contentHeight > 0 && screenW > 0 && screenH > 0) {
        // Integer scaling: find largest integer scale that fits
        int maxScaleX = static_cast<int>(screenW) / contentWidth;
        int maxScaleY = static_cast<int>(screenH) / contentHeight;
        int scale = std::min(maxScaleX, maxScaleY);
        if (scale >= 1) {
            float scaledWidth = contentWidth * scale;
            float scaledHeight = contentHeight * scale;
            scaleX = scaledWidth / screenW * viewportRect.getWidth();
...
void libretrodroid::VideoLayout::updateScreenSize(unsigned int width,unsigned int height) {
    LOGD("Updating screen size: %d x %d", width, height);
...
void libretrodroid::VideoLayout::updateContentSize(unsigned int width, unsigned int height) {
    if (this->contentWidth != width || this->contentHeight != height) {
        LOGD("Updating content size: %d x %d", width, height);
...
void libretrodroid::VideoLayout::setIntegerScaling(bool enabled) {
    if (this->integerScaling != enabled) {
        LOGD("Setting integer scaling: %s", enabled ? "true" : "false");
        this->integerScaling = enabled;
        updateBuffers();
    }
}
